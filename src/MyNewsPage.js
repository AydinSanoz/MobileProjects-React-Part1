...
import { Dimensions, FlatList, Image, SafeAreaView, ScrollView, Text, View,_ScrollView,StyleSheet } from 'react-native';
import {MyNewsCard, MyNewsBanner} from './components';
const news_data = [
  {
      id: 0,
...
const MyNewsPage = () =>{
    ...
    const renderBanner = ({item}) => (<MyNewsBanner bannerData = {item}/>)
    
    const renderNewsData = ({item}) => (<MyNewsCard newsData = {item}/>)
    ...
    return (
        <SafeAreaView>
            <View>
                <Text style = {{fontSize : 50, fontWeight: 'bold'}}> News </Text>
                <FlatList
                    horizontal
                    data = {banner_data}
